...
export function ConversationCard({ conversation }: { conversation: Conversation }) {
  ...
  return (
    <div
      className={`flex items-start gap-4 border-b border-stone-100 bg-white px-4 py-4 transition-colors hover:bg-stone-50 ${
    ...
    >
      <div className="flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-full bg-stone-100">
        {participant?.picture ? (
          <img src={participant.picture} alt="" className="h-full w-full object-cover" />
        ) : (
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-1 flex items-center gap-2">
          <span className="inline-flex items-center gap-1 rounded-md bg-stone-100 px-1.5 py-0.5 text-xs text-stone-500">
            <ChannelIcon channel={conversation.channel} />
            {channelLabel[conversation.channel] ?? conversation.channel}
          </span>
          <span
            className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${statusColor[conversation.status] ?? ""}`}
          >
            {statusLabel[conversation.status] ?? conversation.status}
          </span>
          <span className={`text-xs ${priorityColor[conversation.priority] ?? ""}`}>
            {conversation.priority === "HIGH" ? "!" : "\u00B7"} {conversation.priority}
          </span>
          {isUnread && (
            <span className="text-xs font-medium text-blue-600">
              {conversation.unreadMessagesCount} não {conversation.unreadMessagesCount === 1 ? "lida" : "lidas"}
            </span>
          )}
        </div>
      </div>

      <time className="shrink-0 pt-1 text-xs text-stone-400">
